...
bool Agency::addClient(string name, string address, vector<unsigned int> tour_packs_bought, unsigned int nif, unsigned int family_num) {
  ...
  if(verifyPacks(tour_packs_bought) == false) return false;
...
bool Agency::addClient(Client &client){
  ...
  if(verifyPacks(client.tour_packs_bought) == false) return false;
...
bool Agency::addTravelPack(std::string init_date, std::string final_date, std::string destination, std::vector<std::string> cities, bool available, unsigned int id, unsigned int price, unsigned int people_limit){
  ...
  TravelPack pack(init, fin, destination, cities, available, id, price, people_limit, sumSold(id));

  this->tour_pack.push_back(pack);
  return true;
}
bool Agency::addTravelPack(Date &init_date, Date &final_date, std::string destination, std::vector<std::string> cities, bool available, unsigned int id, unsigned int price, unsigned int people_limit){
  //Check if there is already a pack with the same id
  ...
  TravelPack pack(init_date, final_date, destination, cities, available, id, price, people_limit, sumSold(id));
...
bool Agency::changeClient(Client &client, unsigned int nif){
  ...
  for(size_t i= 0; i < this->client_list.size(); i++){
    if(this->client_list.at(i).nif == nif) {
        ...
        //verify if incrementing is ok
        if(!verifyPacks(client.tour_packs_bought)){
          //verification failled, change back
          for(size_t k = 0; k < this->client_list.at(i).tour_packs_bought.size(); k++){ //decrement number of sold packs
            for(size_t j = 0; j < tour_pack.size(); j++){
              if(tour_pack.at(j).id == this->client_list.at(i).tour_packs_bought.at(k))
                  tour_pack.at(j).num_sold++;
...
unsigned int Agency::sumSold(const unsigned int id){
...
bool Agency::verifyCities(const vector<string> cities){
...
bool Agency::verifyPacks(const vector<unsigned int> &packs){
...
bool Agency::verifyPacksBought(const vector<unsigned int> &packs){
  TravelPack aux;
